...
const sendMessage = async () => {
  ...
  try {
    const response = await api.post(`/conversations/${conversation.value.uuid}/messages`, {
      message: content
    })
    ...
    <template v-else-if="conversation">
      ...
      >
        <template v-for="item in groupedMessages" :key="item.type === 'date' ? item.date : item.id">
          ...
          >
            <div
              :class="[
                'max-w-[80%] sm:max-w-[70%] px-4 py-2.5 shadow-sm',
            ...
            >
              ...
              <div :class="['flex items-center justify-end gap-1 mt-1', item.sender_id === auth.user?.id ? 'text-primary-200' : 'text-slate-400']">
                <span class="text-xs">{{ formatTime(item.created_at) }}</span>
                <!-- Read check for sent messages -->
      ...
      <div class="bg-white border-t border-slate-200 px-4 py-3">
        <div class="flex items-end gap-3 max-w-4xl mx-auto">
          <!-- Attachment Button -->
          <button class="p-2.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition flex-shrink-0">
            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Typing indicator area (for future use) -->
